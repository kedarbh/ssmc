...
function ssmc_custom_primary_menu_fallback()
{
    ...
    echo '<li><a href="' . esc_url(home_url('/contact')) . '" class="hover:text-primary transition py-8 flex items-center">' . esc_html__('Contact', 'ssmc-custom') . '</a></li>';
    echo '</ul>';
}
...
/**
 * Require Event Meta Boxes
 */
require get_template_directory() . '/inc/event-meta-boxes.php';


/**
 * Custom Rewrite Rules for News Category
 */
function ssmc_custom_news_rewrite_rule()
{
...
add_action('init', 'ssmc_custom_news_rewrite_rule');

function ssmc_custom_news_category_link($link, $category_id)
{
    $category = get_category($category_id);
    if ($category && 'news' === $category->slug) {
        return home_url('/news/');
    }
    return $link;
}
add_filter('category_link', 'ssmc_custom_news_category_link', 10, 2);
